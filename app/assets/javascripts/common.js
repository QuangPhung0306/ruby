...
$(document).ready(function () {
    ...
    $('.slick-current.slick-active.slick-center .nav-link').addClass('active');
});
...
$(document).ready(function () {
    ...
    $('body').on('click', '.cal_btnmb', function () {
...
$(document).ready(function () {
    $('body').on('click', '.nav_scrollpy_all li:not(.hidden_pt) a', function () {
        $('html, body').animate({
...
$(function () {
    $('#air_mixture_datetimepicker, #unit_selection_datetimepicker, #datetimepicker').datetimepicker({
        timepicker: false,
        format: 'Y/m/d',
        scrollInput: false
    });
...
$(document).ready(function () {
    ...
    $('.calculation_data_inputs').on('click', '.scrtabs-tab-scroll-arrow-right', function () {
    });

    $('.calculation_data_inputs').on('click', '.scrtabs-tab-scroll-arrow-left', function () {
    });

    $('body').on('click', '.w-tabscroll-ed .css-autow .nav-link', function(){
        $('.slick-slide .item .nav-link.active').removeClass('active');
        $(`.slick-slide .item[data-index=${$(this).parent().data('index')}] .nav-link`).addClass('active');
    });

    $('body').on('click', '.w-tabscroll-ed .css-autow a.nav-link', function() {
        $('.js-tabs-default').text($(this).parent().data('index') + 1);
        $('.slick-slide[current=true]').attr('current', false);
        $(this).parent().parent().parent().attr('current', true);
    });

    $('.calculation_data_inputs').on('click', '.btnNext', function () {
        old_position = $('.slick-slide[current=true] .item').data('index');

        if(old_position + 1 == $('.slick-slide').not('.slick-cloned').length) {
          slick.slick("slickGoTo", 0);
        } else {
          slick.slick("slickGoTo", old_position + 1);
        }

        $('.css-autow .nav-link.active').removeClass('active');

        var current_tab = $('.slick-current.slick-active.slick-center a').addClass('active');
        $('.slick-current.slick-active.slick-center a').click();

        $('.slick-slide[current=true]').attr('current', false);
        $('.slick-current.slick-active.slick-center').attr('current', true);

        var currentPosition = $('.slick-current.slick-active.slick-center .item').data('index') + 1;
        $('.js-tabs-default').text(currentPosition);
        
    });
    $('.calculation_data_inputs').on('click', '.btnPrevious', function () {
        old_position = $('.slick-slide[current=true] .item').data('index');
        
        if(old_position == 0) {
          slick.slick("slickGoTo", $('.slick-slide').not('.slick-cloned').length - 1);
        } else {
          slick.slick("slickGoTo", old_position - 1);
        }
